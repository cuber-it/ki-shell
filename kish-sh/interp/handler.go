...
// SubshellHandlerFunc is a handler for executing subshells via re-exec.
// It receives the shell code to run, the current environment, working directory,
// and stdio. It returns the exit code.
// This allows shells like kish to run subshells as real child processes
// (fork+exec) instead of goroutines, providing real PIDs and signal isolation.
type SubshellHandlerFunc func(ctx context.Context, code string, hc HandlerContext) (uint8, error)
