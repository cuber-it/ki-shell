...
import (
	...
	"github.com/cuber-it/ki-shell/kish-sh/v3/internal"
	"github.com/cuber-it/ki-shell/kish-sh/v3/pattern"
...
func (r *Runner) cmd(ctx context.Context, cm syntax.Command) {
	...
	case *syntax.Subshell:
		if r.subshellHandler != nil {
			// Re-exec: run subshell as a real child process
			var buf strings.Builder
			syntax.NewPrinter().Print(&buf, cm)
			code := buf.String()
			// Strip outer parens from "( cmd )"
			code = strings.TrimPrefix(code, "(")
			code = strings.TrimSuffix(code, ")")
			code = strings.TrimSpace(code)
			hctx := r.handlerCtx(ctx, handlerKindExec, cm.Pos())
			hc := HandlerCtx(hctx)
			exitCode, err := r.subshellHandler(hctx, code, hc)
			if err != nil {
				r.exit.fatal(err)
			} else {
			...
			// Default: goroutine-based subshell
			r2 := r.subshell(false)
