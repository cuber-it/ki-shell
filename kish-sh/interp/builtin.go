...
func (r *Runner) builtin(ctx context.Context, pos syntax.Pos, name string, args []string) (exit exitStatus) {
	...
	case "set":
		if len(args) == 0 {
			// set without args: print all shell variables
	...
	case "shopt":
		...
		for fp.more() {
			...
			case "-o":
				posixOpts = true
			case "-q":
				// Query mode: return 0 if option is set, 1 otherwise
				mode = flag
			case "-p":
				// Print mode: show option settings
				mode = flag
			default:
		...
		for _, arg := range args {
			...
			case "-s", "-u":
				...
				*opt = mode == "-s"
			case "-q":
				// Query: exit 0 if option is set, 1 if unset
				if !*opt {
					exit.code = 1
				}
			case "-p":
				// Print in a form that can be reused as input
				if *opt {
					r.outf("shopt -s %s\n", arg)
				} else {
					r.outf("shopt -u %s\n", arg)
