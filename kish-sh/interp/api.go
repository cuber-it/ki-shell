...
type Runner struct {
	...
	// subshellHandler is an optional function for running subshells via re-exec
	// instead of goroutines. If nil, the default goroutine-based approach is used.
...
// SubshellHandler sets the subshell handler. When set, subshells are executed
// via this handler instead of using goroutines. This allows real process isolation
// via fork+exec (re-exec pattern). See [SubshellHandlerFunc] for more info.
func SubshellHandler(f SubshellHandlerFunc) RunnerOption {
	return func(r *Runner) error {
		r.subshellHandler = f
		return nil
	}
